...
import {getGenDownMarginsData} from './fetchApiData'
...
const plotData = async (targetDateValue:string)=>{
    ...
    if (targetDateValue === "" ) {
        ...
        errorDiv.innerHTML = "";
        try {
                let genDownMarginData:apiRespObj  = await getGenDownMarginsData(
                    targetDateValue
                );
                
                if (genDownMarginData==null)   {
                   errorDiv.classList.add("mt-4", "mb-4", "alert", "alert-danger")
                   errorDiv.innerHTML = "<b>Oops !!! Data Fetch Unsuccessful For Selected Date. Please Try Again</b>" 
                }

                let genDownMarginPlotData: PlotData = {
                    title: `Down Margins ${targetDateValue.substring(0,10)}`,
                    traces: [],
                    yAxisTitle: "--MW--",
                };
                for (const prop in genDownMarginData) {
                   let singleGenMarginTrace: PlotTrace = {
                       name: `${prop}`,
                       data: genDownMarginData[prop],
                       type: "scatter",
                       hoverYaxisDisplay: "MW", 
                    //    fill: 'tonexty',
                       stackgroup: 'one'
                   };
                   genDownMarginPlotData.traces.push(singleGenMarginTrace);
                 } 
                 //line plot for sum Margins
                const allGenList =Object.keys(genDownMarginData)
                ...
                for (let blkInd=0; blkInd<96; blkInd++){
                    ...
                    for (let genInd=0; genInd<allGenList.length; genInd++){
                        ...
                        try{
                            const marginsVal=genDownMarginData[genName][blkInd]['val']
                ...
                genDownMarginPlotData.traces.push(sumScedTrace);
                //setting plot traces
                setPlotTraces(
                    `plotDiv`,
                    genDownMarginPlotData
                );  
